...
def test_hf_data():
    ...
    train_dataset = (
        load_dataset("swag", split="train[:1%]").to_pandas().iloc[0:4]
    )
    dev_dataset = (
        load_dataset("swag", split="train[1%:2%]").to_pandas().iloc[0:4]
    )
    test_dataset = (
        load_dataset("swag", split="test[1%:2%]").to_pandas().iloc[0:4]
    )

    custom_sent_keys = ["sent1", "sent2"]
    label_key = "label"
